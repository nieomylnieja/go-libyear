...
setup_file() {
  ...
  LD_FLAGS="$(
    cat <<EOF
    -X ${PKG}.BuildVersion=2.0.0
    -X ${PKG}.BuildGitTag=v2.0.0
    -X ${PKG}.BuildDate=2023-10-23T08:03:03Z
EOF
  )"
  CLI_BIN_DIR="$BATS_FILE_TMPDIR/bin"
  CLI_BIN="$CLI_BIN_DIR/go-libyear"
  CGO_ENABLED=0 go build \
    -ldflags "$LD_FLAGS" \
    -o "$CLI_BIN" \
    "$BATS_TEST_DIRNAME/../cmd"
  ...
  SERVER_HOST="127.0.0.1"
	go run "$BATS_TEST_DIRNAME/test_server.go" -port "$SERVER_PORT" -path "$INPUTS/responses.json" &
